...
class UVPROJXBuilderForFreeRTOS(object):
    ...
    def create_header(self):
        ...
        print("header file:", output_header_file)
        if 1 or not os.path.exists(output_header_file):
...
int BEEBS();
            ...
            print("hearder: ",output_header_file,self.proj_header)
        else:
    ...
    def build_run(self):
        ...
        # print(axf_file_path)
        if 1 or not os.path.exists(axf_file_path):
            print(
                "\n---------------------\nprepare for project building...", self.proj_name)
            src_main = f'{self.eval_freertos_src_path}{self.proj_name}_main.c'
            # print("copy ", src_main, dst_main)
            # copy_file(src_main, dst_main) #InsectACIDE, disable copy
            # copy_file(src_uvprojx, dst_uvprojx) #InsectACIDE, disable copy
            if os.path.exists(self.build_obj):
                print("clear ", self.build_obj)
                shutil.rmtree(self.build_obj)
            print("ns_project:",self.build_proj_name, self.in_proj_path_ns,
                                 self.out_binary_ns_path, self.out_tmp_path, self.proj_name)
...
class MetadataCreatorForFreeRTOS(object):
    ...
    def create_metadata_run(self):
        ...
        # print("metadata file:", filepath,self.out_metadata_path, self.out_log_path)
