...
{
   // disable systick
   // printf("Disable non-secure systick:%p.\r\n", &(SysTick_NS->CTRL));
   SysTick_NS->CTRL &= ~SysTick_CTRL_ENABLE_Msk; // SYST_CSR
}
...
extern volatile BtHeader *pstBt;
...
ALWAYS_INLINE int TASK_insert(TASK_ST *task, uint32_t item)
{
   ...
   uint32_t record = item;
   int is_user = 0;
   // see whether the task is a user-tasks
   ...
   if (is_user)
   {
      task->task_flag_map[task->num] = task_flag;
   }
   else
   {
      task->task_flag_map[task->num] = 0;
   }
   return task->num++;
}
...
ALWAYS_INLINE int backward_linear_search(uint32_t record, uint32_t *list, int task_num)
{
   if (task_num > 0)
   {
...
ALWAYS_INLINE int CS_TASK_check(TASK_ST *task, uint32_t item, uint32_t just_check)
{
   int idx = backward_linear_search(item, task->task_list, task->num); // get the latest preempted task.

   if (idx < 0)
   {
      return idx;
   }
   int t_flag = task->task_flag_map[idx];
   if (just_check)
   {
      return t_flag;
   }
   ...
   {
      task->task_list[i] = task->task_list[i + 1];
      task->task_flag_map[i] = task->task_flag_map[i + 1];
   }
   task->task_list[task->num] = 0;
   task->task_flag_map[task->num] = 0;
...
void Secure_printf_int(uint32_t num) __attribute__((cmse_nonsecure_entry));
void Secure_printf_int(uint32_t num)
{
   printf("secure_printf_int: %p\r\n", num);
}
...
void Secure_dwt_install_watchpoint(uint32_t comp_id, uint32_t address, uint32_t function) __attribute__((cmse_nonsecure_entry));
void Secure_dwt_install_watchpoint(uint32_t comp_id, uint32_t address, uint32_t function)
{
   dwt_install_watchpoint(comp_id, address, function);
}
...
bool debug_monitor_enable(void)
{
   ...
   // #include "stm32f4xx.h" // Include the appropriate header file that contains the declaration of NVIC_GetPriority.
   ...
   uint32_t priority_timer = NVIC_GetPriority(SysTick_IRQn);
   printf("# %d priority: %d ; # %d timer: %d \r\n", DebugMonitor_IRQn, priority + 1, SysTick_IRQn, priority_timer);
   // for(;;){}
   return true;
}
