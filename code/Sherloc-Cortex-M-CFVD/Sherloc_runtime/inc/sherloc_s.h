...
#define CLEARED_DWT_MASK_VALUE 0x00000004
...
#define SysTick_CTRL_COUNTFLAG_Msk (1UL<<16UL)
...
#define TRACE_BUFF_SIZE 0x10000
...
typedef struct __attribute__((packed))
{
	...
	uint32_t task_flag_map[MAXTASK_LIST];
	int num;
} TASK_ST;
...
extern int CS_TASK_check(TASK_ST *task, uint32_t item,uint32_t just_check);
...
extern int backward_linear_search(uint32_t record, uint32_t *list, int list_size);
/*---------------------------------------------------------------------------
